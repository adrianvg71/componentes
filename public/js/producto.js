window.onload = function() {
  ...
  let abrirModalBtn = document.querySelectorAll("#abrirModal");
  ...
  if(usuario.isLoggedIn) {
    ...
    if(abrirDropdown) {
        ...
        document.addEventListener("mousedown", function(event) {
            if(abrirDropdown) {
                ...
                if (!document.getElementById("cesta-dropdown-content").contains(event.target) && !abrirDropdownCesta.contains(event.target) && !añadirCestaBtn.contains(event.target)) {
    ...
    async function getCarrito() {
      ...
      let nombreProducto = document.querySelector("h1").textContent;
    
      if(carrito.some(carrito => carrito.nombre === nombreProducto)) {
        actualizarProducto(añadirCestaBtn);
      }
  ...
  function actualizarProducto(boton) {
    boton.classList = "flex-grow px-8 py-3 text-gray-200 bg-gray-500/80 hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-opacity-50 rounded-lg text-xl font-semibold cursor-not-allowed opacity-50";
  ...
  let añadirCestaBtn = document.querySelector("#añadirCesta");
  if(añadirCestaBtn){   
  ...
  async function añadirCesta() {
      ...
      let nombreProducto = this.parentNode.parentNode.querySelector("h1").textContent
      try {
  ...
  async function deleteCarrito(id) {
    añadirCestaBtn.classList = "flex-grow px-8 py-3 text-white bg-orange-500/80 hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-opacity-50 rounded-lg text-xl font-semibold";
    añadirCestaBtn.addEventListener("click", añadirCesta);
